...
lbl_result = QLabel("Не знать дефолт, Чел...")
...
questions = [
    ...
    Question('Сколько сегодня градусов?', '10', '-100', '-191116', '532000'),
    Question('Сколько сегодня градусов?', '10', '-100', '-196', '500'),
    Question('Сколько сегодня градусов?', '10', '-5', '-196', '5000')
]
...
def ask(_q):
    ...
    lbl_right_answer.setText(_q.right_answer)
    show_question()

def get_procent(score):
    procent = score / len(question) * 100
    procent = round(procent, 1)
    result = 'Вы ответили на ' + str(score)
    result += ' вопросов из ' + str (len(question)) + '\n'
...
def show_result():
    if is_all_checked():
        if buttons[0].isChecked():
            global score 
            score += 1
            lbl_result.setText('Вы ответити правильно!')
        else:
            lbl_result.setText('Вы ответити неправильно (')
